...
const RECIPE_OPTIONS: Array<{
  ...
  },
  {
    ...
    color: 'border-[#c5a47e]/50 hover:border-[#c5a47e]',
  },
];
...
export const RecipeSelector: React.FC<RecipeSelectorProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative bg-[#111111] border border-white/10 w-full max-w-2xl mx-4 shadow-2xl">
        {/* Header */}
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          {RECIPE_OPTIONS.map((recipe) => (
            <button
              ...
              className={`
                text-left p-5 bg-black/50 border transition-all duration-150
                hover:bg-white/5 group
                ${recipe.color}
              `}
            >
              <div className="flex items-start gap-4">
                <div className="p-2 bg-white/5 text-white/60 group-hover:text-white transition-colors">
                  {recipe.icon}
                </div>
                <div className="flex-1">
                  <h3 className="font-bold text-white group-hover:text-[#c5a47e] transition-colors">
                    {recipe.title}
                  </h3>
