...
export const SlideJournalOverlay: React.FC<SlideJournalOverlayProps> = ({
  ...
  if (relatedEntries.length === 0) {
    return (
      <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
        <div className="bg-white border border-[#D4E5D4] rounded-lg shadow-2xl w-full max-w-md mx-4">
          <div className="flex items-center justify-between px-4 py-3 border-b border-[#D4E5D4]">
            <span className="text-sm font-bold text-[#1E2E1E]">Slide {slideIndex + 1} Insights</span>
            <button
              onClick={onClose}
              className="p-1 text-[#8FA58F] hover:text-[#1E2E1E] transition-colors"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
          <div className="p-6 text-center">
            <Lightbulb className="w-10 h-10 text-[#8FA58F] mx-auto mb-3" />
  ...
  return (
    <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="bg-white border border-[#D4E5D4] rounded-lg shadow-2xl w-full max-w-lg mx-4 max-h-[80vh] overflow-hidden flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-3 border-b border-[#D4E5D4] flex-shrink-0">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 bg-[#6B8E6B]/20 rounded-lg flex items-center justify-center">
              <Lightbulb className="w-3 h-3 text-[#6B8E6B]" />
            </div>
            <span className="text-sm font-bold text-[#1E2E1E]">Slide {slideIndex + 1} Insights</span>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {relatedEntries.map((entry) => (
            <div key={entry.id} className="bg-[#EDF5F0] rounded-lg p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <h4 className="text-sm font-medium text-[#1E2E1E] mb-2">
                    {entry.title}
                  </h4>
                  <p className="text-xs text-[#1E2E1E] leading-relaxed">
                    {entry.narrative}
                  </p>

                  {/* Decision */}
                  {entry.decision && (
                    <div className="flex items-center gap-2 mt-3 pt-3 border-t border-[#D4E5D4]">
                      <Check className="w-3 h-3 text-green-400" />
                      <span className="text-[10px] text-[#8FA58F]">Decision:</span>
                      <span className="text-xs text-[#1E2E1E] font-medium">
                        {entry.decision}
                      </span>
                    </div>
                  )}
                  ...
                  {entry.alternatives && entry.alternatives.length > 0 && (
                    <div className="mt-2">
                      <span className="text-[10px] text-[#8FA58F]">Also considered:</span>
                      <div className="flex flex-wrap gap-1 mt-1">
                        {entry.alternatives.map((alt, i) => (
                          <span
                            key={i}
                            className="text-[10px] text-[#8FA58F] px-2 py-0.5 bg-white rounded-lg"
                          >
                            {alt}
                          </span>
                        ))}
        ...
        {/* Footer */}
        <div className="px-4 py-3 border-t border-[#D4E5D4] flex-shrink-0 bg-[#EDF5F0]">
